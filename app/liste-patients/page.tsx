...
// OOP PatientManager class for data management
class PatientManager {
  ...
  searchPatients(query: string): Patient[] {
    ...
    return this.patients.filter((patient) => {
      ...
      const reverseName = `${patient.prenom} ${patient.nom}`.toLowerCase()
      const id = String(patient.id).toLowerCase()
      const telephone = patient.telephone.toLowerCase()
      const date = patient.date_inscription.toLowerCase()
...
export default function ListePatientsPage() {
  ...
  const handleSearch = useCallback(
    (query: string) => {
      ...
      const results = patientManager.searchPatients(query)
                        ...
                        >
                          ...
                          <td className="whitespace-nowrap px-6 py-4">
                            <span className="text-sm text-muted-foreground">
                              {formatDate(patient.date_inscription)}
                            </span>
                          </td>
                        </tr>
                      ))}
